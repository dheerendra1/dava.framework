...
struct b2EPCollider
{
	...
	void FindIncidentEdge(b2ClipVertex c[2], const b2EPProxy* proxy1, b2_int32 edge1, const b2EPProxy* proxy2);
...
void b2EPCollider::Collide(b2Manifold* manifold)
{
	...
	b2_float32 sideOffset1 = -b2Dot(tangent, v11) + m_radius;
	b2_float32 sideOffset2 = b2Dot(tangent, v12) + m_radius;
...
b2EPAxis b2EPCollider::ComputeEdgeSeparation()
{
	...
	for (b2_int32 i = 0; i < 2; ++i)
	{
		...
		for (b2_int32 j = 0; j < m_proxyB.count; ++j)
		{
			b2_float32 s = b2Dot(n, m_proxyB.vertices[j] - m_edgeA.v1);
...
b2EPAxis b2EPCollider::ComputePolygonSeparation()
{
	...
	axis.index = -1;
	axis.separation = -FLT_MAX;
	for (b2_int32 i = 0; i < m_proxyB.count; ++i)
	{
		...
		b2_float32 s1 = b2Dot(n, m_proxyB.vertices[i] - m_edgeA.v1);
		b2_float32 s2 = b2Dot(n, m_proxyB.vertices[i] - m_edgeA.v2);
...
void b2EPCollider::FindIncidentEdge(b2ClipVertex c[2], const b2EPProxy* proxy1, b2_int32 edge1, const b2EPProxy* proxy2)
{
	b2_int32 count1 = proxy1->count;
	const b2Vec2* normals1 = proxy1->normals;

	b2_int32 count2 = proxy2->count;
