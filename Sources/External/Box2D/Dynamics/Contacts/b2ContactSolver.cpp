...
b2ContactSolver::b2ContactSolver(b2ContactSolverDef* def)
{
	...
	for (b2_int32 i = 0; i < m_count; ++i)
	{
		...
		for (b2_int32 j = 0; j < cc->pointCount; ++j)
		{
...
void b2ContactSolver::InitializeVelocityConstraints()
{
	for (b2_int32 i = 0; i < m_count; ++i)
	{
		...
		b2_float32 radiusA = cc->radiusA;
		b2_float32 radiusB = cc->radiusB;
		...
		b2_float32 wA = bodyA->m_angularVelocity;
		b2_float32 wB = bodyB->m_angularVelocity;
		...
		for (b2_int32 j = 0; j < cc->pointCount; ++j)
		{
			...
			b2_float32 rnA = b2Cross(ccp->rA, cc->normal);
			b2_float32 rnB = b2Cross(ccp->rB, cc->normal);
			rnA *= rnA;
			rnB *= rnB;

			b2_float32 kNormal = bodyA->m_invMass + bodyB->m_invMass + bodyA->m_invI * rnA + bodyB->m_invI * rnB;
			...
			b2_float32 rtA = b2Cross(ccp->rA, tangent);
			b2_float32 rtB = b2Cross(ccp->rB, tangent);
			rtA *= rtA;
			rtB *= rtB;

			b2_float32 kTangent = bodyA->m_invMass + bodyB->m_invMass + bodyA->m_invI * rtA + bodyB->m_invI * rtB;
			...
			ccp->velocityBias = 0.0f;
			b2_float32 vRel = b2Dot(cc->normal, vB + b2Cross(wB, ccp->rB) - vA - b2Cross(wA, ccp->rA));
		...
		if (cc->pointCount == 2)
		{
			...
			b2_float32 invMassA = bodyA->m_invMass;
			b2_float32 invIA = bodyA->m_invI;
			b2_float32 invMassB = bodyB->m_invMass;
			b2_float32 invIB = bodyB->m_invI;

			b2_float32 rn1A = b2Cross(ccp1->rA, cc->normal);
			b2_float32 rn1B = b2Cross(ccp1->rB, cc->normal);
			b2_float32 rn2A = b2Cross(ccp2->rA, cc->normal);
			b2_float32 rn2B = b2Cross(ccp2->rB, cc->normal);

			b2_float32 k11 = invMassA + invMassB + invIA * rn1A * rn1A + invIB * rn1B * rn1B;
			b2_float32 k22 = invMassA + invMassB + invIA * rn2A * rn2A + invIB * rn2B * rn2B;
			b2_float32 k12 = invMassA + invMassB + invIA * rn1A * rn2A + invIB * rn1B * rn2B;

			// Ensure a reasonable condition number.
			const b2_float32 k_maxConditionNumber = 100.0f;
...
void b2ContactSolver::WarmStart()
{
	...
	for (b2_int32 i = 0; i < m_count; ++i)
	{
		...
		for (b2_int32 j = 0; j < c->pointCount; ++j)
		{
...
void b2ContactSolver::SolveVelocityConstraints()
{
	for (b2_int32 i = 0; i < m_count; ++i)
	{
		...
		b2Vec2 normal = c->normal;
		b2Vec2 tangent = b2Cross(normal, 1.0f);
		b2_float32 friction = c->friction;
		...
		for (b2_int32 j = 0; j < c->pointCount; ++j)
		{
			...
			b2_float32 lambda = ccp->tangentMass * (-vt);

			// b2Clamp the accumulated force
			b2_float32 maxFriction = friction * ccp->normalImpulse;
			b2_float32 newImpulse = b2Clamp(ccp->tangentImpulse + lambda, -maxFriction, maxFriction);
		...
		if (c->pointCount == 1)
		{
			...
			b2_float32 lambda = -ccp->normalMass * (vn - ccp->velocityBias);

			// b2Clamp the accumulated impulse
			b2_float32 newImpulse = b2Max(ccp->normalImpulse + lambda, 0.0f);
...
{
	void Initialize(b2ContactConstraint* cc, b2_int32 index)
	{
...
// Sequential solver.
bool b2ContactSolver::SolvePositionConstraints(b2_float32 baumgarte)
{
	...
	for (b2_int32 i = 0; i < m_count; ++i)
	{
		...
		b2_float32 invMassA = bodyA->m_mass * bodyA->m_invMass;
		b2_float32 invIA = bodyA->m_mass * bodyA->m_invI;
		b2_float32 invMassB = bodyB->m_mass * bodyB->m_invMass;
		b2_float32 invIB = bodyB->m_mass * bodyB->m_invI;
		...
		for (b2_int32 j = 0; j < c->pointCount; ++j)
		{
			...
			b2_float32 C = b2Clamp(baumgarte * (separation + b2_linearSlop), -b2_maxLinearCorrection, 0.0f);

			// Compute the effective mass.
			b2_float32 rnA = b2Cross(rA, normal);
			b2_float32 rnB = b2Cross(rB, normal);
			b2_float32 K = invMassA + invMassB + invIA * rnA * rnA + invIB * rnB * rnB;
...
bool b2ContactSolver::SolveTOIPositionConstraints(b2_float32 baumgarte, const b2Body* toiBodyA, const b2Body* toiBodyB)
{
	b2_float32 minSeparation = 0.0f;
	...
	for (b2_int32 i = 0; i < m_count; ++i)
	{
		...
		b2_float32 massA = 0.0f;
		if (bodyA == toiBodyA || bodyA == toiBodyB)
		{
			massA = bodyA->m_mass;
		}

		b2_float32 massB = 0.0f;
