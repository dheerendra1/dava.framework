				...
				b2_float32 r)
{
	...
	b2_float32 C = lengthA + ratio * lengthB;
...
void b2PulleyJoint::InitVelocityConstraints(const b2TimeStep& step)
{
	...
	b2_float32 length1 = m_u1.Length();
	b2_float32 length2 = m_u2.Length();
	...
	b2_float32 C = m_constant - length1 - m_ratio * length2;
	if (C > 0.0f)
	{
	...
	b2_float32 cr1u1 = b2Cross(r1, m_u1);
	b2_float32 cr2u2 = b2Cross(r2, m_u2);
...
void b2PulleyJoint::SolveVelocityConstraints(const b2TimeStep& step)
{
	...
	if (m_state == e_atUpperLimit)
	{
		...
		b2_float32 Cdot = -b2Dot(m_u1, v1) - m_ratio * b2Dot(m_u2, v2);
		b2_float32 impulse = m_pulleyMass * (-Cdot);
	...
	if (m_limitState1 == e_atUpperLimit)
	{
		...
		b2_float32 Cdot = -b2Dot(m_u1, v1);
		b2_float32 impulse = -m_limitMass1 * Cdot;
		b2_float32 oldImpulse = m_limitImpulse1;
	...
	if (m_limitState2 == e_atUpperLimit)
	{
		...
		b2_float32 Cdot = -b2Dot(m_u2, v2);
		b2_float32 impulse = -m_limitMass2 * Cdot;
		b2_float32 oldImpulse = m_limitImpulse2;
...
bool b2PulleyJoint::SolvePositionConstraints(b2_float32 baumgarte)
{
	...
	if (m_state == e_atUpperLimit)
	{
		...
		b2_float32 impulse = -m_pulleyMass * C;
	...
	if (m_limitState1 == e_atUpperLimit)
	{
		...
		b2_float32 C = m_maxLength1 - length1;
		linearError = b2Max(linearError, -C);
		C = b2Clamp(C + b2_linearSlop, -b2_maxLinearCorrection, 0.0f);
		b2_float32 impulse = -m_limitMass1 * C;
	...
	if (m_limitState2 == e_atUpperLimit)
	{
		...
		b2_float32 C = m_maxLength2 - length2;
		linearError = b2Max(linearError, -C);
		C = b2Clamp(C + b2_linearSlop, -b2_maxLinearCorrection, 0.0f);
		b2_float32 impulse = -m_limitMass2 * C;
