...
void b2LineJoint::InitVelocityConstraints(const b2TimeStep& step)
{
	...
	// Prismatic constraint.
	{
		...
		b2_float32 m1 = m_invMassA, m2 = m_invMassB;
		b2_float32 i1 = m_invIA, i2 = m_invIB;

		b2_float32 k11 = m1 + m2 + i1 * m_s1 * m_s1 + i2 * m_s2 * m_s2;
		b2_float32 k12 = i1 * m_s1 * m_a1 + i2 * m_s2 * m_a2;
		b2_float32 k22 = m1 + m2 + i1 * m_a1 * m_a1 + i2 * m_a2 * m_a2;
	...
	if (step.warmStarting)
	{
		...
		b2_float32 L1 = m_impulse.x * m_s1 + (m_motorImpulse + m_impulse.y) * m_a1;
		b2_float32 L2 = m_impulse.x * m_s2 + (m_motorImpulse + m_impulse.y) * m_a2;
...
void b2LineJoint::SolveVelocityConstraints(const b2TimeStep& step)
{
	...
	b2_float32 w1 = b1->m_angularVelocity;
	b2Vec2 v2 = b2->m_linearVelocity;
	b2_float32 w2 = b2->m_angularVelocity;

	// Solve linear motor constraint.
	if (m_enableMotor && m_limitState != e_equalLimits)
	{
		b2_float32 Cdot = b2Dot(m_axis, v2 - v1) + m_a2 * w2 - m_a1 * w1;
		b2_float32 impulse = m_motorMass * (m_motorSpeed - Cdot);
		b2_float32 oldImpulse = m_motorImpulse;
		b2_float32 maxImpulse = step.dt * m_maxMotorForce;
		m_motorImpulse = b2Clamp(m_motorImpulse + impulse, -maxImpulse, maxImpulse);
		impulse = m_motorImpulse - oldImpulse;

		b2Vec2 P = impulse * m_axis;
		b2_float32 L1 = impulse * m_a1;
		b2_float32 L2 = impulse * m_a2;
	...
	b2_float32 Cdot1 = b2Dot(m_perp, v2 - v1) + m_s2 * w2 - m_s1 * w1;

	if (m_enableLimit && m_limitState != e_inactiveLimit)
	{
		// Solve prismatic and limit constraint in block form.
		b2_float32 Cdot2 = b2Dot(m_axis, v2 - v1) + m_a2 * w2 - m_a1 * w1;
		...
		// f2(1) = invK(1,1) * (-Cdot(1) - K(1,2) * (f2(2) - f1(2))) + f1(1)
		b2_float32 b = -Cdot1 - (m_impulse.y - f1.y) * m_K.col2.x;
		b2_float32 f2r;
		if (m_K.col1.x != 0.0f)
		{
		...
		b2_float32 L1 = df.x * m_s1 + df.y * m_a1;
		b2_float32 L2 = df.x * m_s2 + df.y * m_a2;
	...
	else
	{
		...
		b2Vec2 P = df * m_perp;
		b2_float32 L1 = df * m_s1;
		b2_float32 L2 = df * m_s2;
...
bool b2LineJoint::SolvePositionConstraints(b2_float32 baumgarte)
{
	...
	b2_float32 a1 = b1->m_sweep.a;

	b2Vec2 c2 = b2->m_sweep.c;
	b2_float32 a2 = b2->m_sweep.a;

	// Solve linear limit constraint.
	b2_float32 linearError = 0.0f, angularError = 0.0f;
	bool active = false;
	b2_float32 C2 = 0.0f;
	...
	if (m_enableLimit)
	{
		...
		b2_float32 translation = b2Dot(m_axis, d);
		if (b2Abs(m_upperTranslation - m_lowerTranslation) < 2.0f * b2_linearSlop)
		{
	...
	b2_float32 L1 = impulse.x * m_s1 + impulse.y * m_a1;
	b2_float32 L2 = impulse.x * m_s2 + impulse.y * m_a2;
...
b2Vec2 b2LineJoint::GetReactionForce(b2_float32 inv_dt) const
{
	return inv_dt * (m_impulse.x * m_perp + (m_motorImpulse + m_impulse.y) * m_axis);
}

b2_float32 b2LineJoint::GetReactionTorque(b2_float32 inv_dt) const
{
	B2_NOT_USED(inv_dt);
	return 0.0f;
}

b2_float32 b2LineJoint::GetJointTranslation() const
{
	b2Body* b1 = m_bodyA;
	b2Body* b2 = m_bodyB;
...
b2_float32 b2LineJoint::GetJointSpeed() const
{
	b2Body* b1 = m_bodyA;
	b2Body* b2 = m_bodyB;
	...
	b2_float32 speed = b2Dot(d, b2Cross(w1, axis)) + b2Dot(axis, v2 + b2Cross(w2, r2) - v1 - b2Cross(w1, r1));
	return speed;
}
...
b2_float32 b2LineJoint::GetLowerLimit() const
{
	return m_lowerTranslation;
}

b2_float32 b2LineJoint::GetUpperLimit() const
{
	return m_upperTranslation;
}

void b2LineJoint::SetLimits(b2_float32 lower, b2_float32 upper)
{
...
void b2LineJoint::SetMotorSpeed(b2_float32 speed)
{
	m_bodyA->SetAwake(true);
	m_bodyB->SetAwake(true);
	m_motorSpeed = speed;
...
void b2LineJoint::SetMaxMotorForce(b2_float32 force)
{
	m_bodyA->SetAwake(true);
	m_bodyB->SetAwake(true);
...
b2_float32 b2LineJoint::GetMotorForce(b2_float32 inv_dt) const
{
