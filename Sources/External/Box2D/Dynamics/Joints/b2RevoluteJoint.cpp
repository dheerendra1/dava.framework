...
void b2RevoluteJoint::InitVelocityConstraints(const b2TimeStep& step)
{
	...
	b2_float32 m1 = b1->m_invMass, m2 = b2->m_invMass;
	b2_float32 i1 = b1->m_invI, i2 = b2->m_invI;
	...
	if (m_enableLimit)
	{
		b2_float32 jointAngle = b2->m_sweep.a - b1->m_sweep.a - m_referenceAngle;
...
void b2RevoluteJoint::SolveVelocityConstraints(const b2TimeStep& step)
{
	...
	{
		b2_float32 Cdot = w2 - w1 - m_motorSpeed;
		b2_float32 impulse = m_motorMass * (-Cdot);
		b2_float32 oldImpulse = m_motorImpulse;
		b2_float32 maxImpulse = step.dt * m_maxMotorTorque;
...
bool b2RevoluteJoint::SolvePositionConstraints(b2_float32 baumgarte)
{
	...
	if (m_enableLimit && m_limitState != e_inactiveLimit)
	{
		b2_float32 angle = b2->m_sweep.a - b1->m_sweep.a - m_referenceAngle;
	...
	// Solve point-to-point constraint.
	{
		...
		b2_float32 invMass1 = b1->m_invMass, invMass2 = b2->m_invMass;
		b2_float32 invI1 = b1->m_invI, invI2 = b2->m_invI;

		// Handle large detachment.
		const b2_float32 k_allowedStretch = 10.0f * b2_linearSlop;
		if (C.LengthSquared() > k_allowedStretch * k_allowedStretch)
		{
			// Use a particle solution (no rotation).
			b2Vec2 u = C; u.Normalize();
			b2_float32 m = invMass1 + invMass2;
			if (m > 0.0f)
			{
				m = 1.0f / m;
			}
			b2Vec2 impulse = m * (-C);
			const b2_float32 k_beta = 0.5f;
