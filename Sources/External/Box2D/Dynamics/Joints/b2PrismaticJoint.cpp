...
void b2PrismaticJoint::InitVelocityConstraints(const b2TimeStep& step)
{
	...
	// Prismatic constraint.
	{
		...
		b2_float32 k12 = i1 * m_s1 + i2 * m_s2;
		b2_float32 k13 = i1 * m_s1 * m_a1 + i2 * m_s2 * m_a2;
		b2_float32 k22 = i1 + i2;
		if (k22 == 0.0f)
		{
			k22 = 1.0f;
		}
		b2_float32 k23 = i1 * m_a1 + i2 * m_a2;
		b2_float32 k33 = m1 + m2 + i1 * m_a1 * m_a1 + i2 * m_a2 * m_a2;
	...
	if (step.warmStarting)
	{
		...
		b2_float32 L1 = m_impulse.x * m_s1 + m_impulse.y + (m_motorImpulse + m_impulse.z) * m_a1;
		b2_float32 L2 = m_impulse.x * m_s2 + m_impulse.y + (m_motorImpulse + m_impulse.z) * m_a2;
...
void b2PrismaticJoint::SolveVelocityConstraints(const b2TimeStep& step)
{
	...
	if (m_enableLimit && m_limitState != e_inactiveLimit)
	{
		...
		b2_float32 L1 = df.x * m_s1 + df.y + df.z * m_a1;
		b2_float32 L2 = df.x * m_s2 + df.y + df.z * m_a2;
	...
	else
	{
		...
		b2_float32 L1 = df.x * m_s1 + df.y;
		b2_float32 L2 = df.x * m_s2 + df.y;
...
bool b2PrismaticJoint::SolvePositionConstraints(b2_float32 baumgarte)
{
	...
	b2_float32 L1 = impulse.x * m_s1 + impulse.y + impulse.z * m_a1;
	b2_float32 L2 = impulse.x * m_s2 + impulse.y + impulse.z * m_a2;
