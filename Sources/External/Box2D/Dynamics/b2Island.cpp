...
void b2Island::Solve(const b2TimeStep& step, const b2Vec2& gravity, bool allowSleep)
{
	...
	b2_int32 i1 = -1;
	for (b2_int32 i2 = 0; i2 < m_contactCount; ++i2)
	{
	...
	for (b2_int32 i = 0; i < m_bodyCount; ++i)
	{
		...
		b2_float32 rotation = step.dt * b->m_angularVelocity;
...
void b2Island::SolveTOI(const b2TimeStep& subStep, const b2Body* bodyA, const b2Body* bodyB)
{
	...
	const b2_float32 k_toiBaumgarte = 0.75f;
	...
	for (b2_int32 i = 0; i < m_bodyCount; ++i)
	{
		...
		b2_float32 rotation = subStep.dt * b->m_angularVelocity;
		if (rotation * rotation > b2_maxRotationSquared)
		{
			if (rotation < 0.0)
			{
